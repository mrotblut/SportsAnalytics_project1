...
We filtered the dataset to rushing plays from the regular season, ensured IDs were characters, and merged roster information to attach names and positions.
...
We next defined replacement level players within each conference. RB thresholds were set at average roster depth × number of teams; QB thresholds equaled the number of teams in the conference. Players below thresholds were considered replacement level. 
...
We then added contextual variables: Opponent conference, QB run indicator, home field, passing strength of each offense, and opponent defensive stuff rate.
...
From these models, we extracted Impact Per Attempt (IPA) for each player, then multiplied by usage (number of attempts) to compute Impact Per Attempt Accumulated (IPAA).
