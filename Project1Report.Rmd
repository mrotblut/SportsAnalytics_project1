...
We then added contextual variables: Opponent conference, whether or not the play was in the redzone, whether or not the possession team was the home team, passing strength (Passing EPA/Play) of each offense, opponent defensive stuff rate, and point differential before the corresponding play. 
...
From these models, we extracted Individual Points Added (IPA) on the ground for each player, then multiplied by usage (number of attempts) to compute Individual Points Added above Average (IPAA).
...
Our IPAA calculation gave us an indicator of how many individual points an RB and QB added over a global average intercept from a larger population of RB's and QB's. Thus, our next step was to identify a "replacement" level player across each conference to get a better estimate of a player's future impact above or below this replacement level. From our definition of a replacement player in the executive summary, we averaged the IPAA for all players falling below our conference level thresholds for QB's and RB's to determine our next metric, Individual Points Above Average (IPAR), which is calculated by subtracting a player's IPAA by a replacement player (# of attempts x average IPA for replacement player) for each conference, for both QB's and RB's.   
